...
float fireflyLevel(unsigned int rawTime) {
  ...
  for (int i = 0; i < l; i++) {
    ...
    if (time > times[i] && time < times[i + 1]) {
      ...
      float deltaValue = values[i + 1] - values[i];
      int timeSinceLast = time - times[i];
      float ratio = timeSinceLast / (float)deltaTime;
      return ((timeSinceLast * deltaValue) / (float)deltaTime) + values[i];
    }
  }
  return 0.0;
}
